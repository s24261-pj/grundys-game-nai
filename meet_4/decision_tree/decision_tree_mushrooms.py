...
The dataset is assumed to be in CSV format, with features in from second column, first is for labels.
"""
input_file = './../files/agaricus-lepiota.data'
data = np.loadtxt(input_file, delimiter=',', dtype=str)

"""
Each column in the dataset is categorical. 
We use LabelEncoder to convert these categories to numeric values.
"""
...
"""
The first column represents labels (y), and the rest are features (X).
We convert the data into numerical format using `astype`.
"""
...
"""
visualize_classifier(classifier, X_test_pca, y_test_pca, title="Mushrooms Decision Tree Classifier (PCA)", mesh_step_size=0.1)

visualize_classifier(classifier, X_train_pca, y_train_pca, title="Mushrooms Decision Tree Classifier (PCA)", mesh_step_size=0.1)